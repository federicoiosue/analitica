...
public class GoogleAnalyticsHelper extends AnalyticsAbstractHelper {
	...
	GoogleAnalyticsHelper(Application application, String analyticsUrl, String trackingId) {
		super(application, analyticsUrl, trackingId);
		enabled = !TextUtils.isEmpty(analyticsUrl);
		if (tracker == null) {
			tracker = GoogleAnalytics.getInstance(application).newTracker(trackingId);
		}
	}
